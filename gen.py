...
from src.dataset import ImageDataset2
from src.generator import PUNet
from src.evaluate import compute_psnr, compute_ssim
from src import config

def crop_center(img, target_width=960, target_height=720):
    _, width_orig, height_orig = img.shape
...
parser.add_argument('--crop_size', type=int, default=256, help='image training size') #不用管没用
parser.add_argument('--image_root', type=str, default='./data/测试集0705(clip)') #根据需求调整
parser.add_argument('--mask_root', type=str, default='./data/测试集0705(mask2)') #根据需求调整
parser.add_argument('--result_root', type=str, default='./data/测试集0707(final140_mask2)', help='train result') #根据需求调整
parser.add_argument('--number_eval', type=int, default=80, help='number of batches eval') #根据需求调整
args = parser.parse_args()

os.environ['CUDA_VISIBLE_DEVICES'] = '1' #根据需求调整
...
while True:
    ...
    generator.eval()
    for _, (input_images, ground_truths, masks, filenames) in enumerate(data_loader):
        ...
        # save_image(save_images_5, os.path.join(args.result_root, '{:05d}.png'.format(count)), nrow=5)
            target_img = crop_center(outputs[i])
            save_image(target_img, os.path.join(args.result_root, filenames[i]))
